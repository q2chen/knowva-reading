...
- [x] ルートオーケストレーター - 複数エージェントの切り替え・統括（枠組み実装済み、ルーティングロジックは後で追加）

### API・通信
- [x] SSEストリーミング対応 - エージェント応答をリアルタイムにフロントへ配信（`sessions.py`）
    - **注:** ADKの`run_async`は現状トークン単位ストリーミング未対応のため、一括応答。SSE基盤は将来のストリーミング対応に備えて維持。
- [ ] 推薦APIルーター追加（`main.py`）

### フロントエンド
- [x] SSEストリーミング対応のチャットUI（`useStreamingChat.ts`, `StreamingMessageBubble.tsx`）
